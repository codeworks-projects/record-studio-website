...
main_last_local_commit_sha=$(git log -1 --pretty=%H | cat) &&
(git pull || git push --set-upstream origin main ) &&
main_last_origin_commit_sha=$(git log -1 --pretty=%H | cat)

if [ "$main_last_local_commit_sha" != "$main_last_origin_commit_sha" ]; then 
...
main_last_commit=$(git log -1 --pretty=%B | cat)
if [[ "$main_last_commit" == "chore(release)"* ]]; then 
